...
'TranslateCommand("zh", "PH控制-調整", "PH值設定 |4-9|.|00-99|  保持目標酸時間 |0-60|  "), _
'Description("CONTROL PH MAX=PH 9.9,MIN=PH 4.0  ; ADD TIME MAX=60M MIN=FAST  "), _
'TranslateDescription("zh", "PH控制  最高=PH 9.9,最低=PH 4.0  ; 保持目標酸時間 最高=60分 快速=0 ")> _
Public NotInheritable Class Command77
    ...
    Public Function Start(ByVal ParamArray param() As Integer) As Boolean Implements ACCommand.Start
      ...
      .PhControl.Cancel() : .PhWash.Cancel() : .PhControlFlag = False : .PhCirculateRun.Cancel()
            .Command73.Cancel() : .Command74.Cancel() : .Command75.Cancel() : .Command76.Cancel() : .Command78.Cancel() : .Command79.Cancel() : .Command80.Cancel() : .Command81.Cancel() : .Ph76Time.Cancel()
            '---------------------------------------------------------------------------------------------------------
      .F76專用旗標 = False
        ...
        With ControlCode
            Select Case State
                Case S77.Off
                    StateString = ""
                    AddLogTime = False
                    If .Command75.State = Command75.S75.Off And .Command74.State = Command74.S74.Off And .Command76.State = Command76.S76.Off And .Command78.State = Command78.S78.Off And _
