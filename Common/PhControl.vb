Public Class PhControl_
    ...
    Public 實際溫度高於設定溫度flag, 實際溫度高於設定溫度flag2 As Boolean
    ...
    Public Sub Run()
        ...
        Try
            With ControlCode
                Select Case State
                    Case PhControl.Off
                        ...
                        開啟PH控制旗標 = False
                        實際溫度高於設定溫度flag = False
                        實際溫度高於設定溫度flag2 = False
                        If .是否縮短檢測時間 = True Then
                            WaitCheckTime.TimeRemaining = 10
                        Else
              WaitCheckTime.TimeRemaining = .StartCheckPh
            End If
                    ...
                    Case PhControl.DownloadParameter
                        ...
                        If .Command75.State = Command75.S75.Start Then
                            ...
                            If (.IO.MainTemperature > C01TargetTemp) Then
                                ...
                                If 實際溫度高於設定溫度flag = False And 實際溫度高於設定溫度flag2 = False Then
                                    實際溫度高於設定溫度flag = True
                                    實際溫度高於設定溫度flag2 = True
                                ElseIf 實際溫度高於設定溫度flag2 = True And .IO.CallAck Then
                                    實際溫度高於設定溫度flag = False
                                End If
                        ...
                        ElseIf .Command74.State = Command74.S74.Start Then
                            ...
                            State = PhControl.Divider
                            '---------------------------------------------------------------------------------------------------------------------------------------
                        ElseIf .Ph76Time.State = Ph76Time_.Ph76Time.Start Then
            ...
            If .IO.PhValue > (C75Gradient + .PhApproach) Then
              State = PhControl.Divider
            Else
            ...
            '-----------------------------------------------------------------------載入參數(濃度、PH泵加酸比、進水量)
            PhConcentration = CType((.PhConcentration / 100), Double)   '濃度

            '***************************************************************************************2011.9.20   改成X2
            PhPumpOutRatio = CType((.PhPumpOutRatio * 3) / 60, Double) 'PH泵加酸比
            '***************************************************************************************

            '---------------------------此部份如果 進水量 濃度 PH泵加酸比 不得為0,如果為0將修改為1
            If .PhFillLevel = 0 Then

              PhFillLevel = CType(.MainTankFillLevel / 2000, Double)      '進水量
            Else
            ...
            If .IO.PhValue <= (C75Gradient + .PhApproach) Then           'PH實際值 < = PH設定值

              WaitCheckPH.TimeRemaining = .PhSamplingTime              'PH取樣時間
              State = PhControl.CheckPhValue
              Exit Select
            End If
            '------------------------------------
            StateString = ""
                        ...
                        If (((.Command01.Gradient <> 0 Or .Ph76Time.State = Ph76Time_.Ph76Time.Start)) Or .Command80.State = Command80.S80.Start) Then
                            '================================================================================================ 'PH控制模式 = 0
                ...
                Register(3) = TotalAddHac2 - .PhSamplingTime
                If .IO.PhValue > 800 Then
                                    WaitPhAllTime.TimeRemaining = 6
                                ElseIf .IO.PhValue > 700 Then
                                    WaitPhAllTime.TimeRemaining = 5
                                ElseIf .IO.PhValue > 600 Then
                                    WaitPhAllTime.TimeRemaining = 4
                                ElseIf .IO.PhValue > 500 Then
                                    WaitPhAllTime.TimeRemaining = 2
                                ElseIf .IO.PhValue > 450 Then
                                    WaitPhAllTime.TimeRemaining = 1
                                Else
                                    WaitPhAllTime.TimeRemaining = 0
                                End If
...
123:
            ...
            '==================================================================================================20120629起
            If .IO.PhValue < ExpectPh - .PhApproach Or .IO.PhValue < C75Gradient Or WaitAddHac.TimeRemaining <= 0 Then
              OpenHacValve = False
            ...
            If .IO.PhValue > (C75Gradient + .PhApproach) Then                   ''PH實際值 < = PH設定值
              ...
              WaitCheckPH.TimeRemaining = .PhSamplingTime
            ...
            Else
              ...
              If .IO.PhValue <= (C75Gradient + .PhApproach) Then
                ...
                StateString = If(.Language = LanguageValue.ZhTw, "PH值控制 - 結束 ", "PH CONTROL  ")
                State = PhControl.Finished
