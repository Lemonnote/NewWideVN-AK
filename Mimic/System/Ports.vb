...
#End If
      ...
      BitConverter.GetBytes(BaudRate).CopyTo(dcb, 4)
    ...
    Private Sub SetTimeouts()
      Dim ct As New COMMTIMEOUTS With { _
        .ReadIntervalTimeout = readIntervalTimeout_, _
        .ReadTotalTimeoutMultiplier = readTotalTimeoutMultiplier_, _
    ...
    Public Function GetStatistics() As SerialPortStatistics
      Return New SerialPortStatistics With {.BytesIn = stream_.TotalIn, .BytesOut = stream_.TotalOut}
    End Function

    ' -----------------------------------------------------------
    Private Class SerialStream : Inherits System.IO.Stream
...
#End If
  ...
  ' Instead of exceptions, it just returns 0 bytes from Read's
